...
class StandardNodeFactory:
    ...
    def create_simple_node(
        name: str, func: Callable, use_decorator: bool = True
    ) -> Node:
        """通用节点创建工具"""
        if use_decorator:
...
def create_test_node(
    ...
    """
    return StandardNodeFactory.create_simple_node(
        name or func.__name__, func, use_decorator
    )
...
class StandardNodeFactoryExtended(StandardNodeFactory):
    ...
    def create_failing_node(name: str, failure_rate: float = 1.0) -> Node:
        """创建失败节点"""
        config = TestNodeConfig(
            name=name, multiplier=2, should_fail=True, failure_rate=failure_rate
        )
        return StandardNodeFactory.create_processor_node(config)
