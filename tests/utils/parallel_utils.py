...
class ParallelTestValidator:
    ...
    @staticmethod
    def assert_parallel_results(results: Dict[str, ParallelResult], 
                              expected_total: int,
        ...
        # 结果数量检查
        assert len(results) == expected_total, ASSERTION_MESSAGES["count_mismatch"].format(
            expected=expected_total, actual=len(results))
        ...
        if expected_success is not None:
            assert len(successful_results) == expected_success, \
        ...
        if expected_failure is not None:
            assert len(failed_results) == expected_failure, \
                ASSERTION_MESSAGES["count_mismatch"].format(
    ...
    @staticmethod
    def assert_result_keys_unique(results: Dict[str, ParallelResult], 
    ...
    @staticmethod
    def assert_execution_times_reasonable(results: Dict[str, ParallelResult],
                                       min_time: float = 0.0,
                                       max_time: float = 10.0):
        """验证执行时间在合理范围内"""
        for key, result in results.items():
    ...
    def assert_failed_results_have_errors(failed_results: List[tuple]):
        ...
        for key, result in failed_results:
            ...
            assert "intentionally failed" in result.error or "failed" in result.error, \
...
def assert_mixed_results(results: Dict[str, ParallelResult], 
...
def extract_result_values(results: Dict[str, ParallelResult], only_successful: bool = True) -> List[Any]:
    """提取并行结果中的值"""
    values = []
    for key, result in results.items():
