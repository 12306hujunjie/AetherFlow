...
def assert_node_state(context: BaseFlowContext, 
    ...
    if expected_values:
        for key, expected_value in expected_values.items():
            ...
            assert actual_value == expected_value, \
                ASSERTION_MESSAGES["value_mismatch"].format(
...
def assert_test_data_equal(actual: StandardTestData, 
                          expected: StandardTestData,
                          check_timestamp: bool = False,
                          check_metadata: bool = True):
    """断言StandardTestData对象相等"""
    assert isinstance(actual, StandardTestData), \
        ASSERTION_MESSAGES["type_mismatch"].format(
            expected="StandardTestData", actual=type(actual).__name__)
    ...
    if check_metadata and expected.metadata:
        for key, expected_value in expected.metadata.items():
            assert key in actual.metadata, \
                ASSERTION_MESSAGES["key_missing"].format(key=f"metadata.{key}")
...
def assert_user_data_equal(actual: TestUserData, expected: TestUserData):
    """断言TestUserData对象相等"""
    assert isinstance(actual, TestUserData), \
        ASSERTION_MESSAGES["type_mismatch"].format(
...
def assert_container_state_isolation(container: BaseFlowContext,
                                   thread_id: int,
...
def assert_error_contains_message(error: Exception, 
...
def assert_values_in_range(values: List[float], 
                          min_value: float, 
                          max_value: float,
                          value_name: str = "值"):
    """断言所有值都在指定范围内"""
...
def assert_list_contains_types(items: List[Any], expected_types: tuple):
    ...
    for i, item in enumerate(items):
        assert isinstance(item, expected_types), \
            ASSERTION_MESSAGES["type_mismatch"].format(
...
def assert_processing_chain_valid(chain_history: List[Any], 
                                expected_length: int,
                                monotonic_increasing: bool = True):
    """断言处理链历史的有效性"""
    assert len(chain_history) == expected_length, \
        ASSERTION_MESSAGES["count_mismatch"].format(
            expected=expected_length, actual=len(chain_history))
    
    # 检查单调性（如果需要）
    if monotonic_increasing and len(chain_history) > 1:
        for i in range(1, len(chain_history)):
            if isinstance(chain_history[i], (int, float)) and isinstance(chain_history[i-1], (int, float)):
