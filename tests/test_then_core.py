...
# 使用统一的测试基础设施
from .shared.data_models import UserInput, ProcessedUser, FinalResult
...
def test_basic_then():
    ...
    # 使用统一的节点工厂创建节点
    node1 = create_test_node(process_user_input, name="process_user", use_decorator=False)
    node2 = create_test_node(generate_final_result, name="generate_result", use_decorator=False)
...
def test_multiple_chain_combinations():
    ...
    # 测试不同的组合顺序
    pipeline1 = double_node.then(square_node).then(stringify_node)  # 5 -> 10 -> 100 -> "100"
    result1 = pipeline1(5)
    assert result1 == "100"
    print(f"双倍->平方->字符串: 5 -> {result1}")
    
    pipeline2 = square_node.then(double_node).then(stringify_node)  # 5 -> 25 -> 50 -> "50"  
    result2 = pipeline2(5)
    assert result2 == "50"
...
def test_type_validation_in_chains():
    ...
    node1 = create_test_node(strict_int_processor, name="int_to_str", use_decorator=False)
    node2 = create_test_node(strict_str_processor, name="str_to_int", use_decorator=False)
    node3 = create_test_node(strict_final_processor, name="final_check", use_decorator=False)
