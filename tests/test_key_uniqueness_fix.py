...
class KeyTestHelper:
    ...
    def create_simple_node(
        name: str, multiplier: int = 2, should_fail: bool = False
    ) -> Node:
        """创建简单的测试节点"""
        if should_fail:
...
def test_generate_unique_key_no_conflict():
    ...
    existing_results = {"node1": "some_result", "node2": "another_result"}
...
def test_generate_unique_key_sequential_conflicts():
    ...
    assert generated_keys == expected_keys, (
        f"连续键生成不正确: {generated_keys} vs {expected_keys}"
    )
...
def test_parallel_execution_unique_names():
    ...
    assert actual_keys == expected_keys, (
        f"键应该无重复: {actual_keys} vs {expected_keys}"
    )
...
def test_parallel_execution_duplicate_names():
    ...
    actual_keys = set(results.keys())
    assert actual_keys == expected_keys, (
        f"重复名称键处理不正确: {actual_keys} vs {expected_keys}"
    )
    ...
    assert actual_values == expected_values, (
        f"结果值应该不同: {actual_values} vs {expected_values}"
    )
...
def test_parallel_execution_with_failures():
    ...
    actual_keys = set(results.keys())
    assert actual_keys == expected_keys, (
        f"混合场景键处理不正确: {actual_keys} vs {expected_keys}"
    )
    ...
    assert len(successful_results) == 2, (
        f"应该有2个成功结果，实际有{len(successful_results)}个"
    )
    ...
    assert success_values == expected_success_values, (
        f"成功结果值不匹配: {success_values} vs {expected_success_values}"
    )
...
def test_fan_out_fan_in_key_handling():
    ...
    assert result["total_results"] == 3, (
        f"应该处理3个结果，实际处理了{result['total_results']}个"
    )
    assert result["successful_count"] == 3, (
        f"应该有3个成功结果，实际有{result['successful_count']}个"
    )
    assert result["failed_count"] == 0, (
        f"应该有0个失败结果，实际有{result['failed_count']}个"
    )
    ...
    assert result_keys == expected_keys, (
        f"聚合器接收到的键不正确: {result_keys} vs {expected_keys}"
    )
