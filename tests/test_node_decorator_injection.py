...
# 使用统一的测试基础设施
from tests.shared import TestUserData, ProcessedTestData
...
# 模块级依赖注入函数 - 用于复杂场景测试
@node
def process_input_with_injection(data: dict, state: dict = Provide[BaseFlowContext.state]) -> dict:
...
@node
def enhance_data_with_injection(data: dict, context: BaseFlowContext = Provide[BaseFlowContext]) -> dict:
    state = context.state()
    # 读取前一步的state
...
@node
def finalize_result_with_injection(data: dict, context: BaseFlowContext = Provide[BaseFlowContext]) -> dict:
    state = context.state()
...
@node
def state_tracker_with_injection(data: dict, context: BaseFlowContext = Provide[BaseFlowContext]) -> dict:
    state = context.state()
...
def test_then_with_dependency_injection(wired_container):
    ...
    # 构建链式调用
    pipeline = process_input_with_injection.then(enhance_data_with_injection).then(finalize_result_with_injection)
...
@node
def validate_and_process_user(user_data: dict, context: BaseFlowContext = Provide[BaseFlowContext]) -> TestUserData:
    state = context.state()
...
# 节点2: 用户模型转换
@node
def transform_user_model(user: TestUserData,
                         context: BaseFlowContext = Provide[BaseFlowContext]) -> ProcessedTestData:
    state = context.state()
...
def thread_processor_node(data: dict,
    ...
    # 验证state中的线程本地数据仍然存在
    assert state[
               'thread_id'] == thread_id, f"State污染检测：期望thread_id={thread_id}, 实际={state.get('thread_id')}"
